...
const ShelflifeInput = () => {
  ...
  const handleSaveInput = async (input) => {
    try {
      ...
      if (input._id) {
        await axios.put(`http://localhost:3000/shelflife/${input._id}`, input)
        ...
        await axios.post('http://localhost:3000/shelflife', input)
      ...
      const response = await axios.get('http://localhost:3000/shelflife')
  ...
  const handleDeleteInput = async (index) => {
    try {
      ...
      await axios.delete(`http://localhost:3000/shelflife/${idToDelete}`)
